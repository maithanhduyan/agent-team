...
## Verify (candidate improvement)

After install, verify the install is complete and safe:

1. **Confirm the config is readable** (`config/dsh.json` exists and
   parses) and that no plaintext copy was left inside an
   EFS-encrypted directory (`cipher /c` on the target shows no
   unexpected encrypted residue).
2. **Confirm junction safety:** if any installed artifact path is a
   junction link, confirm the link target's contents were never
   touched — only the link itself may have been removed during
   cleanup.
3. **Confirm the service credential is current:** if the service
   account password changed, confirm the service was restarted so the
   new logon takes effect, and that a failed update preserved the
   previous credential (never a partial/corrupt credential that locks
   the service out).

A candidate fix must keep every documented handling directive
(EFS detection/copy/cleanup, junction resolve/traverse/cleanup,
service credential update/restart/failure-safety) intact — this
verification section only adds a post-install confirmation step.
